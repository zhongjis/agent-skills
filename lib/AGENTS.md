...
- `default.nix` — discovery from root `skills/`, vendored common, and physical harness folders; root exclusion; profile splitting; `skillsFor` entry point.
- `exclude-skills.nix` — root-only global exclusion and stale-name validation.
...
- `skill-selection.nix.exclude` removes only named root-authored skills before assembly; stale names throw. Vendored and physical same-name skills remain independently selectable.
