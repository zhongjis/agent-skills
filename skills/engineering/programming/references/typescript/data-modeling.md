...
```
### Boundary API choices

- Use `schema.parse(value)` when invalid data means a broken boundary contract and should throw.
- Use `schema.safeParse(value)` when invalid input is expected and the caller must render or return the validation failure.
- Compose schemas with `.extend()`, `.pick()`, `.omit()`, `.merge()`, and `.partial()` instead of duplicating field definitions.
- Parse environment configuration once at startup, then pass the inferred typed config through the application; do not read `process.env` throughout domain code.
