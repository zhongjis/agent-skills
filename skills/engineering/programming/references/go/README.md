...
Production Go. **Boring on purpose, strict by tooling, illegal states unrepresentable by convention.**
...
| Nil checker | **`nilaway`** in CI | hope |
...
| HTTP framework | **`gin-gonic/gin`** for ecosystem depth — `go-chi/chi` for minimalist, `connectrpc/connect-go` for RPC | `fiber` when stdlib compatibility is required; unmaintained routers |
...
- **HTTP backend** → `backend-stack.md` index: `backend-setup.md`, `backend-middleware.md`, `backend-handlers.md`, `backend-operations.md`.
- **RPC** → `grpc-connect.md` — when to pick Connect vs grpc-go, codegen pipeline, protovalidate, streaming.
- **DB** → `sqlc-pgx.md` index: `sqlc-schema.md`, `sqlc-store.md`, `sqlc-operations.md`.
- **CLI** → `cobra-stack.md` index: `cobra-foundation.md`, `cobra-commands.md`, `cobra-ux.md`.
...
- **Testing** → `testing.md` index: `testing-fundamentals.md`, `testing-integration.md`, `testing-advanced.md`.
