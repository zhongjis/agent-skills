...
With Pydantic v2, use `model_dump_json()` for Pydantic-shaped responses. Adopt orjson where serialization still goes through `dict`, `list`, or `dataclass` values.
...
The shape of the data matters. On mixed-shape APIs, keep `ORJSONResponse` as the default and let Pydantic handle typed responses.
...
Return typed models directly so FastAPI uses Pydantic's serializer:
```python
@app.get("/users/{id}")
...
# Supports every Pydantic serialization feature.
...
Add orjson when profiling or workload shape justifies the dependency.
