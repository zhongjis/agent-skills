...
- `packs.sh` — exercises the root pack runner through synthetic manifests and a fake Skills CLI.
...
- `packs.sh` invokes root `packs.sh` from an arbitrary temp cwd with `PACKS_DIR` fixtures and a `PACKS_SKILLS_BIN` fake. It verifies preflight rejection, ordered grouping and deduplication, exact CLI arguments/cwd/status, and exclusive fake-CLI ownership of lock bytes without any real remote install.
