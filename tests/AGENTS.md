...
- `selector.nix` — asserts root/vendored/physical discovery, canonical authored/adapted research ownership and global common selection, sparse routing, profile outputs, overrides, argument validation, and path validity.
...
- `selector.nix` imports `assembly.nix` and `exclusion.nix`, derives selection expectations from every catalog layer and root config, pins authored/adapted root names including globally common `research`, and preserves routing, profile, path, override, argument, and profile-reference checks.
